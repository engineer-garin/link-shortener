...
# I am aware that, in reality for usability purposes, a service like bitly will use base62 and just leave these out
# but I... don't feel like writing that code for this, so here we are.
_encode = lambda s: base64.urlsafe_b64encode(s).replace('=','')
_decode = lambda s: base64.urlsafe_b64decode(s if len(s) % 2 == 0 else s + '=')
...
class IDFactory(object):
    def __init__(self, secret_key, mac_size, max_id_size):
        ...
        self.min_size = self.mac_factory.mac_size + 1
        self.max_size = self.mac_factory.mac_size + self.max_id_size

    def from_id_num(self, id_num):
        if id_num > self.max_id:
            raise ValueError('ID value %d exceeded max ID value of %d' % (id_num, self.max_id))
    ...
    def from_id_string(self, id_string):
        ...
        id_num = struct.unpack('!Q', id_num_bytes)[0]
...
class ID(object):
    ...
    def __str__(self):
        return '(%d, %s)' % (self.num, repr(self.string))
